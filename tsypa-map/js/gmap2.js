...
(function() {
	...
	this.setMarkers = function() {
		...
		var adressesBlock = jQuery(".adress-block"),
			adress = jQuery(".adress", ".adress-block"),
			data,
			coords,
			spl_coords;
		for (var i = 0; i < adressesBlock.length; i++) {
			data = adress.eq(i).data();
			isCoords = (typeof data.coords != 'undefined');
			if(isCoords){
				spl_coords = data.coords.split(",");
				coords = {
					lat: parseInt(spl_coords[0], 10),
					lng: parseInt(spl_coords[1], 10)
				};
				_addMarker(coords, adressesBlock.eq(i), data.zoom);
			} else {
				_geocoderAddMarker(adress.eq(i).text(), adressesBlock.eq(i), data.zoom);
			}
		};
		return this;
	};
	...
	var _geocoderAddMarker = function(adr, block, zoom) {
		geocoder.geocode({
			...
			if (status == google.maps.GeocoderStatus.OK) {
				_addMarker(result[0].geometry.location, block, zoom);
			} else {
				block.hide();
